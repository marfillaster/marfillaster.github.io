...
state for Route64). The path-agnostic check is one line — run it from a LAN
client after building either path:

```bash
curl -6 -s https://test-ipv6.run/ | head
```

A clean score (or a returned page over v6 at all) means the address plan,
the WireGuard tunnel, and the per-VLAN GUA all work together. Anything less
than that, drop into the path post's verification block to see which layer
is failing.
