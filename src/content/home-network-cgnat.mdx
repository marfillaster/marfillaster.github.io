...
- **[Routed IPv6 over CGNAT via a VPS](/vps-ipv6-cgnat-mikrotik/)** — a
  ...
  the RB5009, and eBGP between them. You operate the endpoint.
...
| **Inbound services on the GUA** | Practical (your firewall, your prefix) | Not advised (best-effort, no SLA) |
| **Failover companion**  | [BGP+BFD post](/mikrotik-ipv6-failover-bgp-bfd/) layers on the BGP session | n/a — single uplink, fast fail-to-IPv4 inside the post |
...
[DNS companion post](/encrypted-dns-stable-resolver-mikrotik/). One detail
is VLAN&nbsp;30-specific: its RA omits the `mtu=1420` clamp the
tunnel-bound VLANs carry, because VLAN&nbsp;30's v6 never reaches a tunnel
and clamping would only shrink local traffic for nothing.*
...
your path post's anti-spoof list:

```bash title="Strip the Guest GUA too"
/ipv6/address remove [find comment="GUEST GUA"]
# also remove the matching guest-legit entry your path post defined
```
