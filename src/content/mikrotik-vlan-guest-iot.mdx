...
[CGNAT build log](/mikrotik-home-network/): split a flat home LAN into a
...
**Hybrid trunks, not a tagged management VLAN.** The AP uplink ports carry
the main LAN *untagged* and only the client SSIDs tagged. APs boot and adopt
on the main LAN with zero VLAN config on the AP side; only client traffic is
tagged. The cost is that the trunk ports are genuinely hybrid (untagged +
tagged on the same wire) and have to be documented as such — which the bridge
VLAN table does.

**Isolation in `forward`, services in `input`.** East-west isolation is one
job (the `forward` chain dropping new flows back into trusted networks) and
"let this VLAN reach the router's DHCP/DNS" is a different job (`input`
accepts). Keeping them in separate chains is what keeps the ruleset
reviewable a year later; starving a VLAN of DHCP to "isolate" it is the
anti-pattern.

**A separate L3 boundary per VLAN.** Each VLAN terminates on its own gateway
address on the router, so inter-VLAN traffic is routed and filterable rather
than bridged. This is what makes the Guest-can't-reach-IoT rule a one-liner.
...
[CGNAT build log](/mikrotik-home-network/) is added later, each VLAN simply
...
stay boring. Only IoT and Guest are tagged, and they share the same uplinks
to the APs — the hybrid-trunk trade-off from the design notes, made concrete
in the bridge VLAN table above.
</Rationale>
...
## References
- [CGNAT build log — the IPv6 / DoH / failover layers on top of this](/mikrotik-home-network/)
