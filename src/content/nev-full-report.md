...
Most of the confusing numbers in this log trace back to one thing: the car draws propulsion energy from three places and meters only one of them. That is a gap in coverage rather than a problem with the instruments — the ones this log uses are accurate. Worth setting out, because it changes which figures mean anything.
...
**Which readings are measurements, and which are estimates.** The gap described above is one of coverage, not accuracy. The three quantities this log is built on are all counted directly and are reliable: the odometer counts distance, the plug-side meter counts mains kilowatt-hours, and the pump counts litres. Nothing about them is approximate. What the car does not provide is a meter on the other two energy paths — regenerative braking and the onboard generator — so the shortfall is in what gets measured, not in how well.

The dashboard's *estimating* readouts are a different matter, and the round trip showed how far they drift. The 35.06 L fill raised the fuel gauge 64 points, but 64 points of a 60 L tank should be 38.4 L, so the gauge reports full about 3.3 L early. Over the return leg the gauge implied 27.6 L consumed where the trip computer's injector count said 22.4 L — a 23% disagreement in the upper part of the tank. Neither is a fault; a float gauge is not a flow meter. But neither belongs in a log. The 14.8 km/L segment figures above are derived from the gauge and should be read as indicative rather than exact, and their distances come from integrating the speed feed, which ran about 5% above the odometer's own count — the odometer was right and the derived figure was not.

**Which is why the fuel side is tank-to-tank.** A fill records exactly how much fuel was consumed since the last one, with no dependence on any estimate. Paired with the odometer and the energy meter, that gives three counted inputs and no inferred ones. Efficiency over a single interval still carries the attribution problem above, since nothing separates regen and generator charge from plug charge, but the distance, fuel, energy and cost totals underneath it are measurements rather than estimates.
...
- [`data/phev_log.csv`](/nev-mileage/data/phev_log.csv) - refuel log (odometer, plug meter, fuel, tariff) backing every figure above
