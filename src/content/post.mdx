...
VPS with a routed /48. DoH and RA RDNSS sit on top of the third layer. None of
these steps depend on a specific brand of fiber service; they only assume that
outbound UDP works and that the router can carry a small WireGuard tunnel.
...
but its only working client (`aiccu`) is unmaintained, was dropped from
Debian for security issues, and has no RouterOS port. WireGuard fills the
same role with a current in-kernel implementation, and the encryption is a
...
for the VPS in this build. The catch is which side initiates the handshake.
Route64 configures peers with a fixed client `Endpoint` and initiates
toward you, so behind CGNAT there is nowhere to point it: the public IPv4
is shared and the client's source port shifts on every WireGuard restart.
The VPS topology inverts that direction — the RB5009 reaches *out* to a
known public IP, and `PersistentKeepalive = 25` keeps the carrier's NAT44
mapping warm. That outbound-initiated direction is the one CGNAT permits,
and owning the reachable side of the tunnel is what the $3/month VPS
actually buys.
...
# Appendix A replaces this with BGP+BFD for ~600 ms.
/ipv6/route add dst-address=::/0 gateway=<LAN_PREFIX>:0::1%wg-host check-gateway=ping
