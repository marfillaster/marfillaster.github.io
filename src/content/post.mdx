...
(sub-second IPv6 failover and cost) and the references
...
(section&nbsp;4.1), and the failover (Appendix&nbsp;A) yours to define
rather than a third party's. The only recurring cost is one cheap VPS
(Appendix&nbsp;B); everything reachable is on an endpoint you own, with no
...
reachable after the WireGuard tunnel handshakes (and, with Appendix&nbsp;A,
after BFD converges). The companion post explains the rest.
</Rationale>
...
- **Metered bandwidth.** The VPS plan is 1&nbsp;TB/month with overage billed
  per Appendix&nbsp;B. High-bitrate video has no reason to transit a rented
...
## B. Appendix B — Cost and provider notes
