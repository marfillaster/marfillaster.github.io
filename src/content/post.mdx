...
/ipv6/nd add interface=vlan30 advertise-dns=yes dns=<ULA_PREFIX>:30::1 \
...
DHCPv4 resolver, so clients take DNS from the ND RDNSS at the ULA, the same as
the main LAN's `defconf` scope. The RA also omits the `mtu=1420` clamp the
tunnel-bound VLANs carry (§5.4) — VLAN&nbsp;30's v6 never reaches the
