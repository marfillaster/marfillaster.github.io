...
order: 9
hero:
  eyebrow: "Series finale · MikroTik RB5009 · multi-homed IPv6"
  description: >-
...
seo:
  dependencies: "MikroTik RouterOS v7, BGP, BFD, WireGuard, Route64, VPS relay"
---
...
from the [Route64 post §2](/route64-ipv6-cgnat-mikrotik/#2-conventions-and-placeholders)
...
The forward chain's `ip6 saddr <OWN_48>::/48` and `ip6 daddr <OWN_48>::/48`
guards are anti-spoof: the VPS will only forward outbound packets sourced
from your prefix and inbound packets destined to it. Without them
`iifname "wg0" accept` would let the home side egress with any source
address, and `oifname "wg0" accept` would let the VPS be used as a
generic router toward whatever happens to be reachable through wg0.

If you configure the public NIC's IPv6 by hand rather than via RA
(`accept_ra=2` above) — on VyOS, static netplan, etc. — give it the
on-link `/64`, never the provider's full `/48`. A `/48` on the
interface installs an on-link route for all 65k `/64`s it covers, so
the box answers its own NDP for addresses that actually live elsewhere
and silently drops them — the multihop transit peer above is exactly
one such address.
</Rationale>
