...
[CGNAT build log](/mikrotik-home-network/), **which is a prerequisite** —
...
cryptokey-routing rationale this depends on — are covered in the base
post's [§4.1](/mikrotik-home-network/#41-return-routing-wireguard-authorizes-bgp-routes).
This post adds only the BFD line and the two edits each side needs to
turn it on.

## 1. Conventions and placeholders
All placeholders are defined in the [CGNAT build's §2](/mikrotik-home-network/#2-conventions-and-placeholders)
— `<LAN_PREFIX>`, `<VPS_AS>` / `<MT_AS>`, `<VPS_ROUTER_ID>` / `<MT_ROUTER_ID>`,
and the `wg-host` interface created in its §4.3. Substitute the same values
here before pasting.
...
mikrotik` block, and add one `wg0`-only allow line to `/etc/nftables.conf`.
...
sed -i '/^protocol bgp mikrotik {/a\  bfd on;' /etc/bird/bird.conf

# 3. Add the wg0-only BFD/3784 allow next to the existing BGP/179 line.
sed -i '/iifname "wg0" tcp dport 179 accept/a\    iifname "wg0" udp dport 3784 accept       # BFD from MikroTik, wg0 only' /etc/nftables.conf
...
## 3. MikroTik — enable BFD on the existing BGP session
...
`established,related,untracked` rule that lets the MikroTik-initiated BGP
session work does not cover them. The explicit allow above is the one new
input rule this companion adds on the MikroTik. The matching nftables line
on the VPS side is in §2.
...
live RB5009, setting only the template did not alter the already-created
...
birdc show route <LAN_PREFIX>::/48         # learned from mikrotik
